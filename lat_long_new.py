...
def database_creation(db_filename, website_url):
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS cityInformation (city TEXT PRIMARY KEY, latitude FLOAT, longitude FLOAT)
    """)
    ...
    for c in tr_tags[1:]:
        ...
        cur.execute("""
        INSERT OR IGNORE INTO cityInformation (city, latitude, longitude)
        VALUES (?,?,?)
        """, (city_name, lat, long)) 
        conn.commit()
