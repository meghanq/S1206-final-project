...
def createQoLCityTable(city_dict, db_filename):
    ...
    cur.execute('''CREATE TABLE IF NOT EXISTS CityQoL (city_count INTEGER PRIMARY KEY, name TEXT, Housing_score NUMBER, Living_Cost_score NUMBER, 
    ...
    cur.execute('SELECT city_count FROM CityQoL WHERE city_count  = (SELECT MAX(city_count) FROM CityQoL)')
    start = cur.fetchone()
    if (start != None):
        start = start[0] + 1
    else:
    ...
    for city in cities[start:start+19]:
        ...
        cur.execute('''INSERT INTO CityQoL (city_count, name, Housing_score, Living_Cost_score, Startup_score, Venture_Capital_score, Travel_score, Commute_score, Business_Freedom_score, 
                    ...
                    Leisure_Culture_score, Tolerance_score, Outdoors_score) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', (city_count, name, Housing_score, 
                    ...
                    Environmental_Quality_score, Economy_score, Taxation_score, Internet_Access_score, Leisure_Culture_score, Tolerance_score, Outdoors_score))
